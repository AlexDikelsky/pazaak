...
fn print_row(p1_row: &[super::card::Card; 3], p2_row: &[super::card::Card; 3]) {
    ...
    for card in p1_row { 
        //TODO FIX THIS
        let i = 0;
        for val in card.get_lines().iter() {
            match i {
            0 => 
...
/*
    /*println!("{}{}{}{}{}{}{}{}{}{}{}{}", EIGHT, p1_row[0], ONE, TOP, ONE, TOP, SEVEN, TOP, ONE, TOP, ONE, TOP);
    ...
    println!();*/
    //for ele in p1_row[0].get_lines().iter() {
    //    println!("{}", ele);
    //}
    
    let mut lines = ["x".green(), "y".green(), "z".green()];
    println!("{:?} HERE", p1_row);
    for ele in p1_row[0].get_lines().iter() {
        for i in 0..3 {
            lines[i] += ele.to_owned();
        }
    }
    for line in lines.iter() {
        println!("{}", line);
    }
    //println!("{}", p1_row[0].get_lines().iter().next().unwrap());
*/
